...
END;
action1 := formatAllMetrics(metrics.states, metricsScope, 'states');
action2 := formatAllMetrics(metrics.counties, metricsScope, 'counties');
action3 := formatAllMetrics(metrics.world, metricsScope, 'world');
trigger:=NOTIFY(EVENT('event4_1', 'success'));
...
ACTIONS:WHEN(EVENT('event3', 'success'), COUNT(100));